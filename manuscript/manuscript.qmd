...
In our `gam_1` model, the `edf` of the smooth term for proficiency is 2.9 and is significantly different from 1 (p \< 2e-16), thus suggesting that the effect of proficiency on accuracy is not linear (Note: the `Ref.df`, reference degrees of freedom, and `F` values merely serve the purpose of being used to derive a *p*-value).
