...
\hypersetup{
  ...
  pdfauthor={Stefano Coretta, Joseph V. Casillas},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
Wieling, 2018)\DIFdelbegin \DIFdel{. This is why we }\DIFdelend \DIFaddbegin \DIFadd{, and biological data (Pedersen et al., 2019). We }\DIFaddend hope
that this tutorial will make GAMMs accessible to researchers from other
fields within linguistics \DIFaddbegin \DIFadd{as well}\DIFaddend . In particular, this tutorial \DIFdelbegin \DIFdel{is thought for researchers in
bilingualism }\DIFdelend \DIFaddbegin \DIFadd{has in
mind researchers focusing on bi- }\DIFaddend and multilingualism who wish to \DIFdelbegin \DIFdel{be able to }\DIFdelend start
using GAMMs for non-linear data, which is \DIFdelbegin \DIFdel{very }\DIFdelend common in developmental and
learning phenomena.
...
1986; Wood, 2006). The main component \DIFdelbegin \DIFdel{that is }\DIFdelend specific to GAMMs \DIFdelbegin \DIFdel{are
smoothers or smooth terms}\DIFdelend \DIFaddbegin \DIFadd{is known as a
smoother or smooth term}\DIFaddend . These are model terms that can fit non-linear
effects. An advantage of using smooth terms is that they are constrained
by a \DIFdelbegin \DIFdel{penalty measure---part }\DIFdelend \DIFaddbegin \DIFadd{smoothing penalty parameter. Importantly, this penalty parameter is
a standard part }\DIFaddend of the fitting procedure \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend does not depend on user
\DIFdelbegin \DIFdel{input---which prevents }\DIFdelend \DIFaddbegin \DIFadd{input and can help prevent }\DIFaddend over-fitting the data. This approach is
different from, for example, growth curve analyses which are based on
user-specified polynomial degrees. \DIFdelbegin %DIFDELCMD <

%DIFDELCMD < %%%
\DIFdel{Readers interested in }\DIFdelend \DIFaddbegin \DIFadd{Note, however, that
under/over-fitting is still possible depending on the number of basis
functions used in building the smoothers (for }\DIFaddend a more detailed
\DIFdelbegin \DIFdel{conceptual discussion of GAMMs are
referred to Sóskuthy (2017).
}\DIFdelend \DIFaddbegin \DIFadd{explanation of basis functions, see the aforementioned tutorials).
}

\DIFaddend The rest of this tutorial focuses on applying GAMMs in bilingualism
research using the statistical programming language R (R Core Team,
2023). \DIFaddbegin \DIFadd{While completing this tutorial will allow you to jump-start into
...
taken at 10 different time points. \DIFaddbegin \DIFadd{Note that the simulated data follow a
...
real learner's data `in the wild' is seldom Gaussian. }\DIFaddend A proficiency
...
We print the first six rows of the data frame below. \DIFaddbegin \DIFadd{Note that text
...
\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Load data into R}
}\DIFaddend \NormalTok{dat1 }\OtherTok{\textless{}{-}} \FunctionTok{readRDS}\NormalTok{(}\StringTok{"data/dat1.rds"}\NormalTok{)}
...
is given in \texttt{subj}. \DIFdelbegin \DIFdel{time point }\DIFdelend \DIFaddbegin \DIFadd{The time point, }\DIFaddend 0 to 9\DIFdelbegin \DIFdel{is in }\DIFdelend \DIFaddbegin \DIFadd{, is in the
}\DIFaddend \texttt{time} \DIFaddbegin \DIFadd{column}\DIFaddend . Figure~\ref{fig-dat1} shows the relationship
between proficiency and learning scores for individual \DIFdelbegin \DIFdel{subjects}\DIFdelend \DIFaddbegin \DIFadd{participants}\DIFaddend .
...
Subsequently, we will analyse the entire data set \DIFdelbegin \DIFdel{, }\DIFdelend to illustrate how to
...
\begin{Shaded}
\begin{Highlighting}[]
\DIFdelbegin \DIFdel{\CommentTok{\# Subset the data to time point 5}
}\DIFdelend \DIFaddbegin \DIFadd{\CommentTok{\# Subset the data to include only time point 5, assign subset to \textquotesingle{}dat15\textquotesingle{}}
}\DIFaddend \NormalTok{dat15 }\OtherTok{\textless{}{-}} \FunctionTok{filter}\NormalTok{(dat1, time }\SpecialCharTok{==} \DecValTok{5}\NormalTok{)}

\DIFdelbegin \DIFdel{\CommentTok{\# attach the mgcv package}
}\DIFdelend \DIFaddbegin \DIFadd{\CommentTok{\# Load the mgcv package}
}\DIFaddend \FunctionTok{library}\NormalTok{(mgcv)}

\DIFdelbegin \DIFdel{\CommentTok{\# fit the model}
}\DIFdelend \DIFaddbegin \DIFadd{\CommentTok{\# Fit the model, assign the model object to \textquotesingle{}gam\_1\textquotesingle{}}
}\DIFaddend \NormalTok{gam\_1 }\OtherTok{\textless{}{-}} \FunctionTok{gam}\NormalTok{(}
  \DIFaddbegin \DIFadd{\AttributeTok{formula =}}\DIFaddend \NormalTok{ score }\SpecialCharTok{\textasciitilde{}} \FunctionTok{s}\NormalTok{(proficiency),}
...
want to estimate a (potentially) non-linear effect \DIFaddbegin \DIFadd{of proficiency}\DIFaddend . The
...
scope of this tutorial. We refer \DIFdelbegin \DIFdel{interested }\DIFdelend readers to Wood (2006) for a
\DIFdelbegin \DIFdel{description
of the underlying math.}\DIFdelend \DIFaddbegin \DIFadd{mathematical description and to other existing tutorials for a gentler
...
\DIFdelbegin \DIFdel{tidygam
package , }\DIFdelend \DIFaddbegin \texttt{\DIFadd{tidygam}} \DIFadd{package (Coretta, 2023), }\DIFaddend which provides users with
utility functions that make extracting and plotting predictions from
GAMs more accessible.

\begin{Shaded}
\begin{Highlighting}[]
\DIFdelbegin \DIFdel{\CommentTok{\# attach tidygam}
}\DIFdelend \DIFaddbegin \DIFadd{\CommentTok{\# Load tidygam package}
}\DIFaddend \FunctionTok{library}\NormalTok{(tidygam)}

\DIFdelbegin \DIFdel{\CommentTok{\# extract model predictions}
}\DIFdelend \DIFaddbegin \DIFadd{\CommentTok{\# Extract model predictions from \textquotesingle{}gam\_1\textquotesingle{}, assign them to \textquotesingle{}gam\_1\_preds\textquotesingle{}}
}\DIFaddend \NormalTok{gam\_1\_preds }\OtherTok{\textless{}{-}} \FunctionTok{predict\_gam}\NormalTok{(gam\_1)}

\DIFdelbegin \DIFdel{\CommentTok{\# plot predictions}
}\DIFdelend \DIFaddbegin \DIFadd{\CommentTok{\# Plot predictions}
}\DIFaddend \FunctionTok{plot}\NormalTok{(gam\_1\_preds, }\AttributeTok{series =} \StringTok{"proficiency"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
details of \DIFdelbegin \DIFdel{predict\_gam() }\DIFdelend \DIFaddbegin \texttt{\DIFadd{predict\_gam()}} \DIFaddend shortly, but for now, notice that
accuracy initially decreases as proficiency increases. At \DIFdelbegin \DIFdel{proficiency }\DIFdelend \DIFaddbegin \DIFadd{a proficiency
score }\DIFaddend of approximately 0.5, accuracy begins to increase, resulting in
the typical U-shaped curve described above. At this juncture we will
inspect the model summary.

\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Print a summary of the model \textquotesingle{}gam\_1\textquotesingle{}}
}\DIFaddend \FunctionTok{summary}\NormalTok{(gam\_1)}
\end{Highlighting}
\end{Shaded}
...
proficiency is 2.9\DIFdelbegin \DIFdel{and is significantly different from 1 (p \textless{}
2e-16)}\DIFdelend , thus suggesting that the effect of proficiency on
...
derive a \emph{p}-value). \DIFaddbegin \DIFadd{The }\emph{\DIFadd{p}}\DIFadd{-value indicates the probability
...
If we were to report this model and \DIFaddbegin \DIFadd{interpret the }\DIFaddend results, we could
...
effect of proficiency is \DIFdelbegin \DIFdel{significantly }\DIFdelend non-linear \DIFaddbegin \DIFadd{and significant }\DIFaddend (\emph{F} = 16.46,
...
relationships involved. \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{Though a }\DIFaddend full treatment of causal inference is
beyond the scope of this paper\DIFdelbegin \DIFdel{. Interested readers }\DIFdelend \DIFaddbegin \DIFadd{, we believe the DAG is informative in
aiding understanding of the model we will build. Readers interested in a
deep dive into causal inference and DAGs }\DIFaddend are referred to McElreath
(2019) \DIFaddbegin \DIFadd{and Pearl (2010)}\DIFaddend .
...
\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Fit model, assign to \textquotesingle{}gam\_2\textquotesingle{}}
}\DIFaddend \NormalTok{gam\_2 }\OtherTok{\textless{}{-}} \FunctionTok{gam}\NormalTok{(}
  \DIFaddbegin \DIFadd{\AttributeTok{formula =}}\DIFaddend \NormalTok{ score }\SpecialCharTok{\textasciitilde{}} \FunctionTok{s}\NormalTok{(proficiency) }\SpecialCharTok{+} \FunctionTok{s}\NormalTok{(time) }\SpecialCharTok{+} \FunctionTok{s}\NormalTok{(subj, }\AttributeTok{bs =} \StringTok{"re"}\NormalTok{),}
...
\texttt{s(\DIFdelbegin \DIFdel{ranint}\DIFdelend \DIFaddbegin \DIFadd{INTERCEPT\_EFFECT}\DIFaddend ,\ bs\ =\ "re")} and random slopes with the
syntax \texttt{s(\DIFdelbegin \DIFdel{ranint}\DIFdelend \DIFaddbegin \DIFadd{INTERCEPT\_EFFECT}\DIFaddend ,\ \DIFdelbegin \DIFdel{ranslope}\DIFdelend \DIFaddbegin \DIFadd{SLOPE\_EFFECT}\DIFaddend ,\ bs\ =\ "re")}. Here
we only include a by-subject random intercept for illustration\DIFaddbegin \DIFadd{, i.e.,
}\texttt{\DIFadd{s(subj,\ bs\ =\ "re")}}\DIFaddend . Below we inspect the model summary.

\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Print summary of model \textquotesingle{}gam\_2\textquotesingle{}}
}\DIFaddend \FunctionTok{summary}\NormalTok{(gam\_2)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Get model predictions for \textquotesingle{}proficiency\textquotesingle{}, assign them to \textquotesingle{}gam\_2\_preds\textquotesingle{}}
}\DIFaddend \NormalTok{gam\_2\_preds }\OtherTok{\textless{}{-}} \FunctionTok{predict\_gam}\NormalTok{(}
\NormalTok{  gam\_2, }\AttributeTok{length\_out =} \DecValTok{25}\NormalTok{,}
  \AttributeTok{series =} \StringTok{"proficiency"}\NormalTok{,}
  \AttributeTok{exclude\_terms =} \FunctionTok{c}\NormalTok{(}\StringTok{"s(subj)"}\DIFdelbegin \DIFdel{\NormalTok{, }\StringTok{"s(subj,proficiency)"}}\DIFdelend \NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\DIFdelbegin %DIFDELCMD < \begin{verbatim}%DIFDELCMD <
%DIFDELCMD < Warning in mgcv::predict.gam(model, newdata = pred_grid, se.fit = TRUE, :
%DIFDELCMD < non-existent exclude terms requested - ignoring
%DIFDELCMD < \end{verbatim}
%DIFDELCMD <

%DIFDELCMD < \begin{verbatim}%DIFDELCMD <
%DIFDELCMD < Warning: There was 1 warning in `dplyr::mutate()`.
%DIFDELCMD < i In argument: `fit = rowSums(dplyr::across())`.
%DIFDELCMD < Caused by warning:
%DIFDELCMD < ! Using `across()` without supplying `.cols` was deprecated in dplyr 1.1.0.
%DIFDELCMD < i Please supply `.cols` instead.
%DIFDELCMD < \end{verbatim}
%DIFDELCMD <
...
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Plot predictions}
}\DIFaddend \FunctionTok{plot}\NormalTok{(gam\_2\_preds)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Get model predictions for \textquotesingle{}time\textquotesingle{}, assign them to \textquotesingle{}gam\_2\_preds\_t\textquotesingle{}}
}\DIFaddend \NormalTok{gam\_2\_preds\_t }\OtherTok{\textless{}{-}} \FunctionTok{predict\_gam}\NormalTok{(}
\NormalTok{  gam\_2, }\AttributeTok{length\_out =} \DecValTok{25}\NormalTok{,}
...
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Plot predictions}
}\DIFaddend \FunctionTok{plot}\NormalTok{(gam\_2\_preds\_t) }\SpecialCharTok{+}
...
of \DIFdelbeginFL \DIFdelFL{proficiency}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{time}\DIFaddendFL , from a GAM.}

\end{figure}%

\DIFaddbegin \DIFadd{Figure~\ref{fig-gam-2-preds} clearly shows a non-linear effect of
...
the model the effect of time is linear but }\emph{\DIFadd{non-zero,}} \DIFadd{hence the
significant }\emph{\DIFadd{p-}}\DIFadd{value for that smooth. The effect is, nonetheless,
...
\DIFadd{We could report this model like so: We fitted a Generalised Additive
...
is non-linear and significant (}\emph{\DIFadd{F}} \DIFadd{= 4638.38, }\emph{\DIFadd{p}} \DIFadd{\textless{}
...
\DIFadd{In this example, we examined u-shaped development in second language
...
outlined }\emph{\DIFadd{a priori}} \DIFadd{using a DAG. We illustrated how to incorporate
non-linear effects into the model via smoothing terms using the function
}\texttt{\DIFadd{s()}} \DIFadd{inside the call to }\texttt{\DIFadd{gam()}}\DIFadd{. Next, we focused on
using the fitted object to generate and plot predictions using
}\texttt{\DIFadd{predict\_gam()}} \DIFadd{and, finally, we walked through how one can
report the resulting analysis. In the next section, we will consider
vowel production in bilingual speech.
}

\DIFaddend \subsection{Case study 2: L2 vowel production in simultaneous and late
...
\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Load data set}
}\DIFaddend \NormalTok{dat2 }\OtherTok{\textless{}{-}} \FunctionTok{readRDS}\NormalTok{(}\StringTok{"data/dat2.rds"}\NormalTok{)}

\DIFaddbegin \DIFadd{\CommentTok{\# Print first 6 rows}
}\DIFaddend \FunctionTok{head}\NormalTok{(dat2)}
\end{Highlighting}
\end{Shaded}
...
\DIFdelbegin \DIFdel{This is what the data looks like.
The top row reports }\DIFdelend \DIFaddbegin \DIFadd{The above code prints the first six rows of the data frame.
Specifically, we observe a subset of }\DIFaddend the synthetic data of \DIFaddbegin \DIFadd{a
}\DIFaddend simultaneous English/Spanish bilinguals (\texttt{es\DIFaddbegin \DIFadd{\_1}\DIFaddend }). It can be
observed \DIFdelbegin \DIFdel{that the }\DIFdelend \DIFaddbegin \DIFadd{in Figure~\ref{fig-dat2} that for the }\texttt{\DIFadd{es}} \DIFadd{group (top
row), the }\DIFaddend Euclidean distance is quite stable across the duration of the
vowel for all three vowels and away from 0 (which corresponds to a
mid-central vowel), indicating no reduction takes place (as \DIFdelbegin \DIFdel{it }\DIFdelend is to be
expected for Spanish). On the other hand, in the second and third row \DIFaddbegin \DIFadd{of
Figure~\ref{fig-dat2}}\DIFaddend , we can see some vowel reduction at play: for /a/,
\DIFdelbegin \DIFdel{beginners }\DIFdelend \DIFaddbegin \DIFadd{beginner }\DIFaddend late bilinguals (\texttt{es\_0}\DIFaddbegin \DIFadd{, middle row}\DIFaddend ) completely reduce
the vowel to a mid-central vowel, while the advanced late bilinguals
(\texttt{es\_1}\DIFdelbegin \DIFdel{) reduce it }\DIFdelend \DIFaddbegin \DIFadd{, bottom row) reduce }\DIFaddend to a lesser degree\DIFdelbegin \DIFdel{, but it is still not quite a full Spanish /a/}\DIFdelend ; /i/ and /u/ are
produced with a diphthongal quality, where the first part of the vowel
is quite reduced. \DIFdelbegin \DIFdel{Finally}\DIFdelend \DIFaddbegin \DIFadd{In sum}\DIFaddend , comparing beginners and advanced bilinguals
shows that the latter have less reduction, \DIFdelbegin \DIFdel{as expected by
}\DIFdelend \DIFaddbegin \DIFadd{presumably a by-product of
}\DIFaddend increasing proficiency in Spanish.
...
We can use a GAM to model \DIFaddbegin \DIFadd{the }\DIFaddend Euclidean distance along time \DIFdelbegin \DIFdel{point }\DIFdelend \DIFaddbegin \DIFadd{points }\DIFaddend in
...
argument inside the smooth term function \texttt{s()}\DIFdelbegin \DIFdel{: for example}\DIFdelend \DIFaddbegin \DIFadd{. For example,
}\DIFaddend \texttt{s(timep,\ by\ =\ vowel)} would ensure a non-linear effect of
time point is estimated for each vowel. In our data, we want to model
the interaction of vowel (/a, i, u/) and group
(\texttt{es,\ es\_0,\ es\_1}). Unfortunately, \DIFdelbegin \DIFdel{classical interactions
cannot be model in GAMs (because of the ``additive '' aspect of
}\DIFdelend the \DIFdelbegin \DIFdel{model; }\DIFdelend \DIFaddbegin \DIFadd{additive nature of
GAMs does not allow the direct specification of interactions between
smooth terms like one would do in a linear model (}\DIFaddend interactions require
product operations). \DIFaddbegin \DIFadd{Note that interactions between parametric terms are
...
To \DIFdelbegin \DIFdel{obviate this limitation}\DIFdelend \DIFaddbegin \DIFadd{include interactions between smooth terms}\DIFaddend , we can simply construct a
...
linear term). \DIFaddbegin \DIFadd{We would also like to allow for the modelled trajectories
to vary by subject both in height and shape. While random effects
included with }\texttt{\DIFadd{bs\ =\ "re"}} \DIFadd{(which we used in the first case
study) account for difference in height and rotation of the fitted
curves, }\emph{\DIFadd{random factor smooth interaction}} \DIFadd{terms fit a different
curve to each level of the chosen factor (in our case, subject). Random
factor smooth interaction terms are specified with }\texttt{\DIFadd{bs\ =\ "fs"}}\DIFadd{:
the first argument in }\texttt{\DIFadd{s()}} \DIFadd{should be the variable to smooth over
(here, }\texttt{\DIFadd{timep}}\DIFadd{) and the second argument is the factor (here
}\texttt{\DIFadd{subj}}\DIFadd{), the levels of which will be fitted separately. Finally,
the }\texttt{\DIFadd{m}} \DIFadd{argument corresponds to the order of the smoothing
penalty: higher orders lead to more smoothing (the default is
}\texttt{\DIFadd{2}}\DIFadd{, a second-order penalty). Setting }\texttt{\DIFadd{m\ =\ 1}}
\DIFadd{corresponds to a higher constraint on the smooths of each level in the
random factor, and it is thus a safeguard against over-fitting.
}\DIFaddend

\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Generate group\_vowel column to model the interaction}
}\DIFaddend \NormalTok{dat2 }\OtherTok{\textless{}{-}}\NormalTok{ dat2 }\DIFdelbegin \DIFdel{\SpecialCharTok{\%\textgreater{}\%}
  }\DIFdelend \DIFaddbegin \DIFadd{\SpecialCharTok{|\textgreater{}}
  }\DIFaddend \FunctionTok{mutate}\NormalTok{(}
...
\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Get model predictions, assign them to object \textquotesingle{}gam\_3\_preds\textquotesingle{}}
}\DIFaddend \NormalTok{gam\_3\_preds }\OtherTok{\textless{}{-}} \FunctionTok{predict\_gam}\NormalTok{(gam\_3, }\AttributeTok{exclude\_terms =} \StringTok{"s(timep,subj)"}\NormalTok{)}
...
\DIFdelbegin \DIFdel{function. And we might also want to }\DIFdelend \DIFaddbegin \DIFadd{`}\texttt{\DIFadd{predict\_gam()\textasciigrave{}}} \DIFadd{function. Additionally, we can
also }\DIFaddend extract more than 10 time points \DIFdelbegin \DIFdel{, }\DIFdelend to get a smoother curve.

\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\CommentTok{\# Get model predictions, separate group\_vowel column}
}\DIFaddend \NormalTok{gam\_3\_preds\_2 }\OtherTok{\textless{}{-}} \FunctionTok{predict\_gam}\NormalTok{(}
\NormalTok{  gam\_3,}
...
\DIFaddbegin \DIFadd{What follows is an example of how the model and results could be
reported in prose.
}

\DIFadd{We fitted a Generalised Additive Model to Euclidean Distance values
...
}\texttt{\DIFadd{group\_vowel}} \DIFadd{which is the combination of group (}\texttt{\DIFadd{es}}\DIFadd{,
}\texttt{\DIFadd{es\_0}}\DIFadd{, }\texttt{\DIFadd{es\_1}}\DIFadd{) and vowel (}\texttt{\DIFadd{a}}\DIFadd{, }\texttt{\DIFadd{i}}\DIFadd{,
}\texttt{\DIFadd{u}}\DIFadd{) to model mean differences in EUD between group/vowel, and a
smooth term over time point (}\texttt{\DIFadd{timep}}\DIFadd{) with }\texttt{\DIFadd{group\_vowel}}
\DIFadd{as a by-variable to model non-linear trajectories of EUD in each
...
\DIFadd{The simultaneous English/Spanish bilingual data (group }\texttt{\DIFadd{es}}\DIFadd{) in
...
late bilinguals (group }\texttt{\DIFadd{es\_0}}\DIFadd{), the plot indicates that /a/ is
reduced to }{[}\DIFadd{ə}{]} \DIFadd{(the EUD is 0), that /i/ and /u/ are strongly
diphthongised, and that the first part of the diphthong is close to
}{[}\DIFadd{ə}{]} \DIFadd{while the second part does not quite reach the EUD values of the
simultaneous English/Spanish bilinguals, again as we would expect in
English speakers learning Spanish.
}

\DIFadd{In the high-proficiency late English/Spanish bilinguals we can observe
...
\subsection{\DIFadd{Summary}}\label{summary}

\DIFadd{This introductory tutorial covered the basics of fitting non-linear
...
\DIFadd{Two literature-based simulated case studies were employed to illustrate
how to fit and plot GAMs. Case study 1 was about U-shaped learning and
focused on modelling non-linear effects with smooths (}\texttt{\DIFadd{s()}} \DIFadd{terms
...
study introduced readers to }\texttt{\DIFadd{by}}\DIFadd{- variables, used to model
...
\DIFadd{This tutorial on its own will not be sufficient to develop an adequate
...
\DIFaddbegin \bibitem[\citeproctext]{ref-coretta2023}
\DIFadd{Coretta, S. (2023). }\emph{\DIFadd{Tidygam: Tidy prediction and plotting of
generalised additive models}}\DIFadd{.
}\url{/url\%7Bhttps://CRAN.R-project.org/package=tidygam\%7D}

\DIFaddend \bibitem[\citeproctext]{ref-ervin_miller1963}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-gigerenzer2004}
\DIFadd{Gigerenzer, G. (2004). Mindless statistics. }\emph{\DIFadd{The Journal of
Socio-Economics}}\DIFadd{, }\emph{33}(5)\DIFadd{, 587--606.
}\url{https://doi.org/10.1016/j.socec.2004.09.033}

\DIFaddend \bibitem[\citeproctext]{ref-hastie1986}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-pearl2010introduction}
\DIFadd{Pearl, J. (2010). An introduction to causal inference. }\emph{\DIFadd{The
International Journal of Biostatistics}}\DIFadd{, }\emph{6}(2)\DIFadd{.
}\url{https://doi.org/10.2202/1557-4679.1203}

\bibitem[\citeproctext]{ref-pedersen2019}
\DIFadd{Pedersen, E. J., Miller, D. L., Simpson, G. L., \& Ross, N. (2019).
Hierarchical generalized additive models in ecology: An introduction
with mgcv. }\emph{\DIFadd{PeerJ}}\DIFadd{, }\emph{\DIFadd{7}}\DIFadd{, e6876.
}\url{https://doi.org/10.7717/peerj.6876}

\DIFaddend \bibitem[\citeproctext]{ref-pliatsikas2013processing}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-simpson2023}
\DIFadd{Simpson, G. (2023). }\emph{\DIFadd{What do p-values mean in generalized additive
model?}} \DIFadd{Cross Validated.
}

\DIFaddend \bibitem[\citeproctext]{ref-soskuthy2017}
...
{R}}. CRC Press\DIFaddbegin \DIFadd{.
}

\bibitem[\citeproctext]{ref-wood2011}
\DIFadd{Wood, S. (2011). Fast stable restricted maximum likelihood and marginal
likelihood estimation of semiparametric generalized linear models.
}\emph{\DIFadd{Journal of the Royal Statistical Society (B)}}\DIFadd{, }\emph{73}(1)\DIFadd{,
3--36}\DIFaddend .
